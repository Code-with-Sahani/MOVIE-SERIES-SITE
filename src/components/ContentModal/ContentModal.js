...
export default function TransitionsModal({ children, media_type, id }) {
  ...
  const fetchData = async () => {
    const { data } = await axios.get(
      `https://api.themoviedb.org/3/${media_type}/${id}?api_key=d6ad1aaa6dfb51cf110ae18829f59aa2&language=en-US`
    );
  ...
  const fetchVideo = async () => {
    const { data } = await axios.get(
      `https://api.themoviedb.org/3/${media_type}/${id}/videos?api_key=d6ad1aaa6dfb51cf110ae18829f59aa2&language=en-US`
    );
  ...
  return (
    <>
      <div
        className="media" style={{ cursor: "pointer" }} color="inherit" onClick={handleOpen}
      >
        {children}
      </div>
